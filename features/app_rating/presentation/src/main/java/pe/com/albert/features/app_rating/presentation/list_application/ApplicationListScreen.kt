...
@Composable
fun ApplicationListScreen(goToForm: (String) -> Unit = {}) {
    ...
    ListScreenContent(applications = state.applications, onClick = {
        viewModel.setIntent(ApplicationListUiIntent.GoToForm(it.id))
    })

    LaunchedEffect(viewModel.navigation) {
        viewModel.navigation.collect {
            when (it) {
                is ApplicationListNavigation.GoToForm -> goToForm(it.id)
            }
        }
    }
}
